...
DO I = 1 TO SRC.0
  ...
  IF POS('WORKING-STORAGE',UCODE) > 0 &,
    ...
    IF ADDED_SQLCA = 0 THEN DO
      ...
      /* V3: Generate proper host variables from metadata */
      CALL GENERATE_HOST_VARIABLES
      CALL ADD_CODE,
...
/*============================================================*/
/* GENERATE_HOST_VARIABLES: Emit WS host vars for each file   */
/* Only primary (non-REDEFINES) fields are emitted.           */
/* Group items become 01-level records for the SQL INTO clause */
/*============================================================*/
GENERATE_HOST_VARIABLES:
  DO GF = 1 TO FILE.0
    FN = FILE.GF.NAME
    IF FLD.FN.P.0 = 0 THEN ITERATE
    CALL ADD_CODE '      * V2D2: HOST VARIABLES FOR' FN
    CALL ADD_CODE '      * V2D2: Primary fields only -',
      FLD.FN.RCOUNT 'REDEFINES skipped'
    /* Emit a 01-level group for this file's record */
    RECNAME = 'V2-' || LEFT(FN,6) || '-REC'
    CALL ADD_CODE '       01  ' || RECNAME || '.'
    /* Emit each primary elementary field */
    DO GI = 1 TO FLD.FN.P.0
      FNAME = FLD.FN.P.GI.NAME
      FPIC = FLD.FN.P.GI.PIC
      FUSAGE = FLD.FN.P.GI.USAGE
      FLVL = FLD.FN.P.GI.LEVEL
      /* Use level 05 for all fields under the group */
      FDEF = '           05  ' || FNAME
      /* Pad to column 40 for PIC alignment */
      IF LENGTH(FDEF) < 40 THEN
        FDEF = LEFT(FDEF,40)
      FDEF = FDEF || ' PIC ' || FPIC
      /* Add USAGE if not DISPLAY */
      IF FUSAGE = 'COMP-3' THEN
        FDEF = FDEF || ' COMP-3'
      ELSE IF FUSAGE = 'COMP' THEN
        FDEF = FDEF || ' COMP'
      FDEF = FDEF || '.'
      CALL ADD_CODE FDEF
    END
